...
Prefer simple expiry and refresh-on-use for contact metadata unless measurements justify more machinery. Keep user aliases immediately consistent and independent of cache expiry. A process may retain hot lookups while connected to MCP, but persistence must also work across process restarts.
