...
public enum MCPServerRunner {
    ...
    public static func run(operations: MessagesOperations, transport: any Transport) async throws {
        let server = Server(name: "messages-swift", version: "0.1.0", instructions: "Read, search and count local Messages with selected Contacts names, local aliases and message-bound images/files. Ambiguous contacts require a choice. Drafting never calls send_message. Show resolved recipients, exact text and files and obtain confirmation. An unchanged confirmed preview needs no second conversational confirmation; any recipient, content or file change requires a revised preview. Send to the resolved explicit handle or exact chatID; never resolve a contact name again after approval. Omit service for automatic history-based transport selection. Groups retain their exact existing chat. resolve_send_route is optional diagnostic information. The client approves the invocation. For unfamiliar phones, the operation uses the SMS relay when available, otherwise iMessage, and may use one eligible alternative only after a proven pre-dispatch route failure. Never retry an uncertain or partial send. sent is a uniquely correlated post-dispatch source-row observation, not an AppleScript receipt or delivery guarantee; a source-confirmed failure is a tool error. Attachment tools return bounded image views or complete original file bytes; errors do not deliver a file. Decoding diagnostics mean search coverage is incomplete. Never interpret a cached label as a send destination or route guarantee.", capabilities: .init(tools: .init()))
