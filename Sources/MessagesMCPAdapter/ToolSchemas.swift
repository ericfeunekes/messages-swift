...
enum ToolSchemas {
    ...
    static let tools: [Tool] = [
        ...
        Tool(name: "resolve_send_route", description: "Read-only direct/group route resolution for exactly one existing chatID or one recipient selector. A direct result lists currently available services and a history-based suggestion, which is not proof of present recipient capability. Resolve before previewing a direct send; never probe transports by sending.", inputSchema: object(["chatID": string, "recipients": directRecipient]), annotations: .init(readOnlyHint: true, openWorldHint: false)),
        Tool(name: "send_message", description: "Send only after confirmation of an exact recipients/service/text/files preview, including when the initial request says send. Drafting never calls this tool. Exactly one destination: an existing group chatID without service, or a direct chatID/one explicit recipient handle with an explicit frozen service. Use resolve_send_route before previewing a direct send; its suggestion is a hint, not capability proof. After confirmation, send to the resolved explicit handle or exact direct chatID with that frozen service; never re-resolve a contact name. Ambiguous names/handles return choices. Text then files in order, stops on rejection/unknown; never retry or change transport automatically. sent means a uniquely correlated post-dispatch source row (unique_source_match), not an AppleScript receipt or guaranteed attribution; delivered is separate. A known source failure is a tool error.", inputSchema: object(["chatID": string, "recipients": directRecipient, "service": ["type": "string", "enum": ["iMessage", "SMS", "RCS"]], "text": string, "files": ["type": "array", "items": string]]), annotations: .init(readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true)),
