#!/bin/zsh
set -euo pipefail
(( $# == 1 )) || { print -u2 "usage: $0 start|stop|start-app|stop-app|start-tunnel|stop-tunnel|status|status-app|status-tunnel"; exit 64; }
...
case "$1" in start) start "$app"; start "$tunnel";; stop) stop "$tunnel" || exit $?; stop "$app";; start-app) start "$app";; stop-app) stop "$app";; start-tunnel) start "$tunnel";; stop-tunnel) stop "$tunnel";; status) presence "$app" && presence "$tunnel";; status-app) presence "$app";; status-tunnel) presence "$tunnel";; *) exit 64;; esac
